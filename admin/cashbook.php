...
$stmt = $pdo->prepare("SELECT tt.id, 'thanh_toan' AS loai, CONCAT('Thanh toán bàn ', b.so_ban) AS danh_muc, tt.tong_tien AS so_tien, tt.phuong_thuc, tt.ma_tham_chieu, tt.ghi_chu, tt.created_at, a.ho_ten, a.username FROM thanh_toan tt JOIN ban b ON b.id=tt.ban_id JOIN admin a ON a.id=tt.collected_by WHERE 1=1" . $scopePayment . " ORDER BY tt.created_at DESC LIMIT 30");
$stmt->execute($params);
$payments = $stmt->fetchAll();
$stmt = $pdo->prepare("SELECT tc.id, tc.loai, tc.danh_muc, tc.so_tien, NULL AS phuong_thuc, NULL AS ma_tham_chieu, tc.ghi_chu, tc.created_at, a.ho_ten, a.username FROM thu_chi tc JOIN admin a ON a.id=tc.created_by WHERE 1=1" . $scopeCash . " ORDER BY tc.created_at DESC LIMIT 30");
$stmt->execute($params);
