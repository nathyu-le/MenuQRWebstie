...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'open_shift') {
            ...
            $stmt = $pdo->prepare("INSERT INTO ca_thu_ngan (opened_by, opening_cash) VALUES (?, ?)");
...
<div class="admin-layout"><?php require __DIR__ . '/_sidebar.php'; ?><main class="admin-content">
    ...
    <section class="table-card"><h3>Lịch sử ca</h3><div class="responsive-table"><table class="table"><thead><tr><th>Nhân viên</th><th>Trạng thái</th><th>Đầu ca</th><th>Kỳ vọng</th><th>Thực tế</th><th>Lệch</th></tr></thead><tbody><?php foreach ($shifts as $shift): ?><tr><td><?= htmlspecialchars($shift['ho_ten'] ?: $shift['username']) ?><small class="ledger-method"><?= htmlspecialchars($shift['opened_at']) ?></small></td><td><span class="role-badge <?= $shift['trang_thai']==='dang_mo'?'role-cashier':'role-manager' ?>"><?= $shift['trang_thai']==='dang_mo'?'Đang mở':'Đã chốt' ?></span></td><td><?= number_format((float)$shift['opening_cash'],0,',','.') ?>đ</td><td><?= $shift['expected_cash']===null?'—':number_format((float)$shift['expected_cash'],0,',','.').'đ' ?></td><td><?= $shift['closing_cash']===null?'—':number_format((float)$shift['closing_cash'],0,',','.').'đ' ?></td><td class="money <?= (float)$shift['discrepancy']<0?'expense':'income' ?>"><?= $shift['discrepancy']===null?'—':number_format((float)$shift['discrepancy'],0,',','.').'đ' ?></td></tr><?php endforeach; ?></tbody></table></div></section></div>
</main></div></body></html>
